...
private var CustomInputViewHandle: UInt8 = 0
extension SectionEditorWebView {
    ...
    override open var inputView: UIView? {
        get {
            return getCustomInputView()
        }
        set {
            setCustomInputView(newValue)
        }
    }

    @objc private func setCustomInputView(_ customView: UIView?) {
        objc_setAssociatedObject(self, &CustomInputViewHandle, customView, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC)
        ...
        guard let newClass = classWithCustomView(targetView: targetView) else {
            assertionFailure("Couldn't get class")
            return
        }
        object_setClass(targetView, newClass)
    ...
    private func classWithCustomView(targetView: UIView) -> AnyClass? {
        let customViewClassName = "\(ContentViewPrefix)_CustomInputView"
        ...
        add(
            ...
            to: newClass.self,
            origin: SectionEditorWebView.self,
            originSelector: #selector(getCustomInputView)
        )
    ...
    @objc private func getCustomInputView() -> UIView? {
        let webView = wmf_firstSuperviewOfType(SectionEditorWebView.self)
        return objc_getAssociatedObject(webView as Any, &CustomInputViewHandle) as? UIView
    }
}
