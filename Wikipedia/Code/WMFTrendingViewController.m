...
-(NSArray<NSString*>*)titlePrefixExclusions {
    return @[
             ...
             @"Special:"    // Is this same in every lang?
             ];
}
...
-(void)getTrendingTitles {
    ...
    [[[WMFTrendingFetcher alloc] init] fetchTrendingForSite:self.site date:[NSDate date]]
    .then(^(NSArray* results) {
        return results;
    })
    .catch(^(NSError* error){
        // Then fallback to previous day's date if failure.
        // Needed because trending data is compiled once daily, and local date may or may not be +1 days from last trending compilation date.
