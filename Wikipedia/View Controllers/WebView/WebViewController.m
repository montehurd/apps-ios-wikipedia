...
@property (nonatomic, strong) WMFBarButtonItem* buttonTOC;
@property (nonatomic, strong) WMFBarButtonItem* buttonBack;
@property (nonatomic, strong) WMFBarButtonItem* buttonForward;
@property (nonatomic, strong) WMFBarButtonItem* buttonLanguages;
@property (nonatomic, strong) WMFBarButtonItem* buttonSave;
@property (nonatomic, strong) WMFBarButtonItem* buttonShare;
...
- (void)setupTopMenuButtons {
    ...
    self.buttonTOC = [[WMFBarButtonItem alloc] initBarButtonOfType:WMF_BUTTON_TOC_COLLAPSED
                                                           handler:^(id sender){
        [self tocToggle];
    }];

    self.buttonTOC.selectedType  = WMF_BUTTON_TOC_EXPANDED;
    self.buttonTOC.selectedColor = [UIColor blackColor];
...
- (void)setupBottomMenuButtons {
    ...
    self.buttonBack = [[WMFBarButtonItem alloc] initBarButtonOfType:WMF_BUTTON_BACKWARD handler:^(id sender){
        goBeforeOrAfter(@"before");
    }];
    self.buttonForward = [[WMFBarButtonItem alloc] initBarButtonOfType:WMF_BUTTON_FORWARD handler:^(id sender){
        goBeforeOrAfter(@"after");
    }];
    self.buttonLanguages = [[WMFBarButtonItem alloc] initBarButtonOfType:WMF_BUTTON_TRANSLATE handler:^(id sender){
        [self showLanguages];
    }];

    self.buttonSave = [[WMFBarButtonItem alloc] initBarButtonOfType:WMF_BUTTON_HEART_OUTLINE handler:^(id sender){
        [self saveCurrentPage];
        [self updateBottomBarButtonsEnabledState];
    }];
    self.buttonSave.selectedType  = WMF_BUTTON_HEART;
    self.buttonSave.selectedColor = UIColorFromRGBWithAlpha(0xf27072, 1.0);

    self.buttonShare = [[WMFBarButtonItem alloc] initBarButtonOfType:WMF_BUTTON_SHARE handler:^(id sender){
        NSLog(@"TEST 4");
    }];
...
- (void)updateTOCButtonVisibility {
    self.buttonTOC.disabled = [SessionSingleton sharedInstance].currentArticle.isMain;
}
...
- (void)updateBottomBarButtonsEnabledState {
    ...
    self.buttonLanguages.disabled = self.session.currentArticle.isMain && [self.session.currentArticle languagecount] > 0;
    self.buttonSave.selected      = [self isCurrentArticleSaved];
}
