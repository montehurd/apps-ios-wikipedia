...
+ (LoginViewController*)initialViewControllerFromStoryBoard {
    return [[UIStoryboard storyboardWithName:@"WMFLogin" bundle:nil] instantiateInitialViewController];
}
...
- (void)save {
    ...
    [self loginWithUserName:self.usernameField.text
        ...
        [self performSelector:(onboardingVC ? @selector(dismissParent) : @selector(dismissSelf)) withObject:nil afterDelay:1.2f];
    } onFail:^{
...
- (void)createAccountButtonPushed:(UITapGestureRecognizer*)recognizer {
    if (recognizer.state == UIGestureRecognizerStateEnded) {
        ...
        AccountCreationViewController* createAcctVC = [AccountCreationViewController initialViewControllerFromStoryBoard];
