...
- (void)uploadWithManager: (AFHTTPRequestOperationManager *)manager
{
    ...
    [manager POST:url parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if(![responseObject isDict]){
            responseObject = @{@"error": @{@"info": @"Wikidata description not uploaded."}};
        }
        ...
        if (!error && result && [result isEqualToString:@"Failure"]) {
            ...
            // Set error condition so dependent ops don't even start and so the errorBlock below will fire.
            error = [NSError errorWithDomain:@"Wikidata Description Uploader" code:WIKIDATA_UPLOAD_ERROR_FAILURE userInfo:errorDict];
        }
