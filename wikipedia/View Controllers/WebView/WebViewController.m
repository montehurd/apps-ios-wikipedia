...
- (void)viewDidLoad {
    ...
    [self.bridge addListener:@"DOMContentLoaded" withBlock:^(NSString* type, NSDictionary* payload) {
        ...
        [weakSelf.fadedLoadingIndicator fadeToTransparent];
...
- (void)tocShow {
    ...
    if (![self.fadedLoadingIndicator isTransparent]) {
        return;
    }
...
- (void)  navigateToPage:(MWKTitle*)title
    ...
    if (showLoadingIndicator) {
        BOOL showSpinner = discoveryMethod != MWK_DISCOVERY_METHOD_BACKFORWARD;
        [self.fadedLoadingIndicator fadeFromTransparentToColor:[UIColor whiteColor] alpha:0.8f useSpinner:showSpinner];
    }
    ...
    if ([sender isKindOfClass:[ArticleFetcher class]]) {
        ...
        switch (status) {
            ...
            case FETCH_FINAL_STATUS_FAILED:
            {
                ...
                [self.fadedLoadingIndicator fadeToTransparent];
...
- (void)loadingIndicatorAdd {
    self.fadedLoadingIndicator = [[WMFFadedLoadingIndicator alloc] init];
    [self.view insertSubview:self.fadedLoadingIndicator belowSubview:self.bottomBarView];
    [self.fadedLoadingIndicator mas_makeConstraints:^(MASConstraintMaker* make) {
        make.edges.equalTo(self.fadedLoadingIndicator.superview);
    }];
}
