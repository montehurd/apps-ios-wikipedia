...
-(NSDictionary *)getOptionTextBaseAttributes
{
    ...
    return @{
             ...
             NSForegroundColorAttributeName : UIColorFromRGBWithAlpha(kBaseTextColor, 1.0),
...
{
    self.langGlyphLabel.backgroundColor = UIColorFromRGBWithAlpha(kLangGlyphBackgroundColor, 1.0);

    [self.langGlyphLabel setWikiText: WIKIGLYPH_TRANSLATE
                               color: UIColorFromRGBWithAlpha(kLangGlyphForgroundColor, 1.0)
...
-(NSAttributedString *)getAttributedStringForOptionLanguagesWithCount:(NSInteger)count
{
    ...
    [langButtonString attributedStringWithAttributes: [self getOptionTextBaseAttributes]
               ...
               substitutionAttributes: @[@{NSForegroundColorAttributeName : UIColorFromRGBWithAlpha(kLangCountColor, 1.0)}]];
}
...
-(void)updateLastModifiedDate:(NSDate *)date userName:(NSString *)userName
{
    ...
    self.lastModGlyphLabel.backgroundColor = UIColorFromRGBWithAlpha(kLastModGlyphBackgroundColor, 1.0);
    [self.lastModGlyphLabel setWikiText: WIKIGLYPH_PENCIL
                                  color: UIColorFromRGBWithAlpha(kLastModGlyphForgroundColor, 1.0)
                                   size: kGlyphButtonFontSize * MENUS_SCALE_MULTIPLIER
                         baselineOffset: 1.7];
}
...
-(NSAttributedString *)getAttributedStringForOptionLastModifiedByUserName:(NSString *)userName date:(NSDate *)date
{
    ...
    [lastModString attributedStringWithAttributes: [self getOptionTextBaseAttributes]
                           ...
                           substitutionAttributes: @[
                                                     @{NSForegroundColorAttributeName : UIColorFromRGBWithAlpha(kLastModTimestampColor, 1.0)},
                                                     @{NSForegroundColorAttributeName : UIColorFromRGBWithAlpha(kLastModUsernameColor, 1.0)}
                                                     ]];
}
